(function ($) {
    ...
    $.fn.ssunnyEjGrid = function (options) {
                    ...
                    $("#" + _thisId).off().on("click", ".e-headercell", function(args) {
                        ...
                        for (var i = 0; i < length; i++) {
                            if (gridObj.model.columns[i]["headerText"] == headerText) {
                                ...
                                // window[gridObj._id].orderChange(columnData);
        ...
        this.sortingData = function(cItem) {
            ...
            //var requestData = $.extend(this.opt.postData, this.ejGridOption.requestGridData);

            if(this.ejGridOption.requestGridData.sord === 'desc') {
                console.log('sord: desc');
                this.ejGridOption.requestGridData.sord = 'asc';


            } else {
                console.log('sord: asc');
                this.ejGridOption.requestGridData.sord = 'desc';
            }

            //this.ejGridOption.sortorder = cItem.
        }

        /*
        this.setDataBinding = function(){
        	...
        	$.ajax({
        		...
        		success: function(result) {
        			console.log(result);
        			_this.opt.gridControl.dataSource(result.rows);
        		},
        		error: function(xhr, status, error) {
        			console.log(error);
        		},
        ...
        */
        ...
        this.rowBoxInit = function(){
            ...
            selectbox.setData(arr, function(obj){
                _this.pageInit();
                _this.ejGridOption.rowNum = obj.key;
        ...
        this.searchBoxInit = function(){
            ...
            for( var i = 0; i< this.ejGridOption.columns.length; i++ ){
                if( this.ejGridOption.columns[i].visible != false && ( this.ejGridOption.columns[i].customAttributes.searchable == null || this.ejGridOption.columns[i].customAttributes.searchable == true) ){
                    headArr.push( {value: this.ejGridOption.columns[count].headerText, key : this.ejGridOption.columns[i].customAttributes.index }); //colModel은 hidden이 있으므로 count로 indexing
                }
